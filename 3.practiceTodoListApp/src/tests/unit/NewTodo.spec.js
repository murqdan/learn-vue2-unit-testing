...
describe('Page: NewTodo', () => {
  it('shows error message when btn click and form incomplete', async () => {
    ...
    expect(wrapper.findComponent({ ref: 'errorMessage' }).exists()).toBeTruthy()
  })

  it('redirects to TodoList when save data success and status 201', async () => {
    ...
    await wrapper.findComponent({ ref: 'button' }).trigger('click')
    expect(wrapper.findComponent({ ref: 'button' }).attributes('disabled')).toBeTruthy()
    expect(wrapper.findComponent({ ref: 'errorMessage' }).exists()).toBeFalsy()
  ...
  it('shows error message when save data success and status not 201', async () => {
    axios.post.mockResolvedValue({status: 500, message: 'internal server error'})
    setup()
